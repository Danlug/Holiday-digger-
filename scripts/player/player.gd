...
const HH := 1.0 / 3.0
# Допуск скругления углов — доля клетки, на которую хитбоксу разрешено
# перехлестнуть за угол, чтобы соскользнуть, а не встать честным блоком (см.
# _move_x/_move_y). 7 px был подобран ДО правки HH выше, под старую высоту
# хитбокса 0.92 клетки (29.4 px) — те 7 px составляли ~24% высоты хитбокса.
# С новым HH=1/3 (высота 0.67 клетки = 21.3 px) те же 7 px — это уже ~33%
# высоты, и ровно поэтому герой стал соскальзывать по углам заметно чаще и
# сильнее (владелец, 2026-09-22: «проскальзывает углы слишком сильно») —
# геометрически у меньшего хитбокса гораздо шире окно позиций-у-кромки, где
# ровно один из двух щупов (верх/низ) цепляет соседнюю клетку, а другой нет
# (именно это условие и включает скольжение). Дело не в CORNER как числе, а
# в его ДОЛЕ от уменьшившегося хитбокса — возвращаем ту же ~24%-долю: 7 px
# было подобрано под 29.4 px высоты именно потому, что при НЕЙ 5 px (доля
# ~17%) не хватало и герой застревал (см. историю константы в git log) — тот
# дефицит был у старой, высокой коробки. У новой, низкой 5 px — это те же
# ~24%, что раньше работали без сцепления, поэтому допуск возвращается к
# 5 px, а не остаётся 7.
const CORNER := 5.0 / 32.0
# tests/test_player_harness.gd:_test_corner_rounding_* — форгив до 5 px
# проскальзывает, 6+ px по-прежнему честно блокирует.
...
func teleport_home() -> void:
	# Смерть, «часы-телепорт» и отладочная кнопка «Дом» могут застать героя ЗА
	# РУЛЁМ бурмобиля под землёй. В отличие от обычного выезда на поверхность
	# (_start_rig_exit/_finish_rig_transition), телепорт не паркует машину —
	# GameState.rig_parked_at остаётся (-1,-1). Если после такого телепорта
	# игрок наденет в мастерской другой инструмент (это разрешено —
	# house_is_indoors снимает блокировку «бурмобиль под землёй не бросить»),
	# бурмобиль потеряется НАВСЕГДА: кнопка «В бурмобиль» и посадка требуют
	# GameState.is_rig_parked(), а другого пути снова сделать current_tool ==
	# "drill_rig" в игре нет — куплен за 10000 монет и материалы, и потерять
	# его можно молча, одной покупкой гаджета. Поэтому телепорт обязан
	# "довезти" машину и высадить героя ровно как обычный выезд: запарковать
	# у устья тоннеля (то же место, что и _start_rig_exit) и переключить на
	# лучший инструмент в собственности. player_depth выставляется в 0 ДО
	# смены инструмента — set_current_tool блокирует смену «под землёй», а
	# после телепорта герой уже физически на поверхности.
	if is_in_rig():
