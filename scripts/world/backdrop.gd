...
## Низ горы прикреплён к картинке леса с забором по высоте (задание
## владельца от 2026-09-21: «картинка с горой на фоне должна быть
## прикреплена по высоте к лесу с забором») — считается от FENCE_BOTTOM_Y
## (низ картинки забора), а не отдельным подобранным числом: изменится
## высота забора — гора поедет вместе с ним, а не разъедется.
##
## Раньше бралась привязка к FENCE_TOP_Y (верху картинки) — на глаз выглядит
## логично ("верх леса — верх горы"), но у backdrop_mountain.png низ САМОЙ
## картинки — не голая порода, а собственный нарисованный луг с лесом на
## переднем плане (см. art/env/backdrop_mountain.png): у FENCE_TOP_Y этот
## луг горы ещё не докуда прячется — крона забора там едва начинается
## (проверено по альфе backdrop_fence_white.png: сплошной непрозрачный
## полог — только с ~60 текстурных px, т.е. ближе к НИЗУ картинки забора).
## Экран показывал одновременно и чужой луг горы, и лес забора рядом — два
## явно разных рисунка леса встык, а не одна гора, спрятанная за лесом.
## FENCE_BOTTOM_Y — низ ВСЕЙ картинки забора (64 логических px высотой) —
## гарантирует, что низ горы утонет под сплошной кроной забора целиком,
## а не только заденет её верхнюю, местами прозрачную кромку.
const MOUNTAIN_BOTTOM_WORLD_Y := FENCE_BOTTOM_Y   # 5
const MOUNTAIN_BOTTOM_SCREEN_Y := MOUNTAIN_BOTTOM_WORLD_Y - MOUNTAIN_REF_CAM_Y_PX  # 213
const MOUNTAIN_TOP_SCREEN_Y := MOUNTAIN_BOTTOM_SCREEN_Y - MOUNTAIN_H_LOGICAL  # 53
