...
## Гора БОЛЬШЕ НЕ закреплена по экрану (владелец, 2026-09-21: «когда прыгаю,
## видно проблеск между лесом и горой, открепи картинку с горой от фона»).
## Раньше update() гасила сдвиг камеры по Y ПОЛНОСТЬЮ — гора висела на
## фиксированном экранном пикселе, а забор (как и весь остальной мир) едет
## по Y вместе с камерой 1:1. В долю секунды прыжка camera.y успевает
## сдвинуться — забор от этого чуть съезжает вниз/вверх экрана, а
## неподвижная гора нет, и в щели между их нижним/верхним краем на кадр
## проглядывает небо. По X параллакс (MOUNTAIN_PARALLAX_X) остаётся —
## жалобы на него не было, там смещение плавное и медленное, глаз не ловит
## рывок за один кадр, как с внезапным прыжком по Y.
##
## Теперь гора едет по Y как забор — 1:1 с миром (_mountain_layer.position.y
## всегда 0, см. update()), и её нижний край задаётся МИРОВОЙ координатой,
## той же системой отсчёта, что и FENCE_TOP_Y/FENCE_BOTTOM_Y, а не
## экранным пикселем через опорную камеру. Раз оба слоя двигаются
## одинаково, щель между ними исчезает при любом движении камеры, а не
## только в опорном положении "герой стоит у себя в огороде".
##
## Число — FENCE_BOTTOM_Y минус 25 логических px (задание владельца:
## «прикрепи по высоте -25 пикселей к лесу на фоне»): FENCE_BOTTOM_Y —
## низ ВСЕЙ картинки забора, под сплошной непрозрачной кроной (у
## FENCE_TOP_Y крона ещё местами прозрачна — см. историю правок ниже),
## −25 от него открывает чуть больше горы над лесом, чем полное
## прилипание к самому низу картинки.
const MOUNTAIN_BOTTOM_WORLD_Y := FENCE_BOTTOM_Y - 25.0   # −20
const MOUNTAIN_TOP_WORLD_Y := MOUNTAIN_BOTTOM_WORLD_Y - MOUNTAIN_H_LOGICAL  # −180

## Гору эта константа больше не позиционирует (см. коммент выше — гора
## теперь в обычных мировых координатах, как забор). Оставлена ради
## sky_view.gd: там своё, ОТДЕЛЬНОЕ и намеренно сохранённое закрепление по
## экрану — солнце/месяц/звёзды по-прежнему держатся "опорной высоты
## стояния героя" (её самой жалобы не было, трогать нечего), и им нужен
## тот же опорный camera.y, что раньше калибровал гору. Число — cam.y
## стоящего у себя на огороде героя (портрет 224×480, экран 14 клеток
## высотой, y=0.5 — см. scripts/ui/hud.gd:get_view_cells).
...
func update(cam: Vector2) -> void:
	...
	if _mountain_layer != null:
		...
		# движения камеры (плавный параллакс, жалоб не было). По Y — ничего не
		# гасим, 0: гора едет вместе с миром 1:1, как забор (см. коммент
		# константы MOUNTAIN_BOTTOM_WORLD_Y выше — раньше гасили полностью, и
		# при прыжке между горой и забором на кадр проглядывало небо).
		_mountain_layer.position = Vector2((1.0 - MOUNTAIN_PARALLAX_X) * cam_px.x, 0.0)
...
func _draw_mountain() -> void:
	...
	while x < x_end:
		_mountain_layer.draw_texture_rect(_tex_mountain,
				Rect2(x, MOUNTAIN_TOP_WORLD_Y, MOUNTAIN_W_LOGICAL, MOUNTAIN_H_LOGICAL), false)
