...
## Низ горы прикреплён к картинке леса по высоте (задание владельца от
## 2026-09-21: «опусти картинку горы на 20 пикселей и прикрепи к картинке
## леса по высоте») — считается прямо от FENCE_TOP_Y (верх картинки забора
## с лесом), а не отдельным подобранным числом: изменится высота леса —
## гора поедет вместе с ним, а не разъедется. +20 логических px ниже
## верхней кромки леса — нижний край картинки горы уходит чуть за кроны
## деревьев, а не повисает точно на их макушках.
const MOUNTAIN_BOTTOM_WORLD_Y := FENCE_TOP_Y + 20.0   # −39
const MOUNTAIN_BOTTOM_SCREEN_Y := MOUNTAIN_BOTTOM_WORLD_Y - MOUNTAIN_REF_CAM_Y_PX  # 169
const MOUNTAIN_TOP_SCREEN_Y := MOUNTAIN_BOTTOM_SCREEN_Y - MOUNTAIN_H_LOGICAL  # 9
