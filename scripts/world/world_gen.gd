...
#
# ВАЖНО (отчёт агента, баг владельца "не копается даже справа от лестницы"):
# самый широкий уровень лестницы — САМЫЙ ПЕРВЫЙ (y=1, прямо под ногами героя
# на старте), а не четвёртый/нижний. Раньше было наоборот (лестница
# сужалась СВЕРХУ ВНИЗ: 1 клетка на y=1, 4 клетки на y=4) — герой видел на
# поверхности только один клин лестницы шириной в клетку, шагал "направо от
# него" на x=16..18 и упирался в молчаливый отказ: гейт держал границу по
# правому краю ЧЕТВЁРТОГО уровня (x=19), а глазами с поверхности казалось,
# что лестница уже кончилась. Теперь лестница сужается СВЕРХУ ВНИЗ в
# противоположную сторону (широкая у поверхности, к низу — одна клетка), и
# вся запертая полоса видна игроку сразу, одним взглядом, ещё до первой
# попытки копать — см. _permanent_feature_type.
...
func _permanent_feature_type(x: int, y: int) -> int:
	...
	if _garden_locked:
		...
		#
		# Ширина по уровню — (y_max - y + 1): самая широкая ступень на y=1
		# (прямо на поверхности, во всю ширину гейта _pre_autodig_min_x),
		# дальше вниз лестница сужается к одной клетке на самом нижнем
		# уровне. Раньше было наоборот (широко внизу, узко на поверхности) —
		# герой на старте видел лишь одну клетку лестницы и не понимал,
		# почему явно правее неё копать всё ещё нельзя (см. отчёт агента у
		# _pre_autodig_min_x выше). Сумма ширины по всем уровням та же, что
		# и раньше (1+2+..+y_max) — от разворота фигуры лестница не меняется
		# в размере, меняется только то, где именно виден её широкий край.
		if era != "grandpa" and x >= _staircase_x_start \
				and x < _staircase_x_start + (_staircase_y_max - y + 1):
