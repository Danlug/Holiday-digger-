...
# уровне: _staircase_x_start + _staircase_y_max. Пересчитывается в
# _apply_layers вместе с самой лестницей, а не хардкодится — расходиться с
# геометрией лестницы ей нельзя.
#
# ВАЖНО (владелец, 2026-09-21: «лестницу ты зря развернул, я не могу
# вылезти»): широкий конец лестницы обязан быть ВНИЗУ (y=4 — самый нижний,
# самый ранний уровень), узкий — НАВЕРХУ (y=1, под ногами героя на старте).
# Только так столбцы читаются как настоящая лестница: у самого дома (x=15)
# сплошная порода начинается сразу с y=1 — это первая, самая верхняя
# ступень, — а дальше от дома (x=16, 17, 18) сплошная порода начинается всё
# глубже (y=2, 3, 4), и над каждой такой ступенью остаётся открытый воздух
# до самого верха — по нему герой и поднимается. Разворот "широко наверху,
# узко внизу" (был в этом файле раньше — отчёт агента про баг "не копается
# даже справа от лестницы") даёт обратное: сплошная порода перекрывает ВЕСЬ
# верхний уровень (y=1) по всей ширине лестницы разом — герой, оказавшись
# под ней, упирается в потолок без единого прохода и не может выбраться
# наружу (баг владельца выше). Гейт "до автокопки — только правее лестницы"
# (см. is_pre_autodig_locked_cell) от ориентации самой лестницы не зависит —
# он и так держит границу x=19 везде, как бы она ни была нарисована.
...
##
## До сцены Роберта тоннеля в мире нет вовсе (решение владельца) —
## Vector2i(-1, -1) значит "нет тоннеля". Все нынешние вызывающие (дом) уже
## проверяют GameState.house_hatch_built ДО обращения сюда, так что это —
## подстраховка, а не смена контракта: ни один существующий путь её раньше
## не требовал, но новый код, который забудет проверку, получит явный "нет",
## а не координаты клетки, которая ещё не построена.
...
func _permanent_feature_type(x: int, y: int) -> int:
	...
	if _garden_locked:
		...
		#
		# Ширина по уровню — y (не y_max-y+1, см. отчёт владельца у
		# _pre_autodig_min_x выше): широкий конец ВНИЗУ (y=4, x=15..18),
		# узкий НАВЕРХУ (y=1, x=15 только). Это и есть форма настоящей
		# лестницы, а не перевёрнутый клин: у дома (x=15) сплошная порода
		# начинается сразу под ногами (y=1) — первая ступень, — а дальше от
		# дома каждый следующий столбец (x=16, 17, 18) открыт воздухом
		# глубже и глубже, до y=2, 3, 4 — по этому воздуху герой и
		# поднимается наверх, ступень за ступенью.
		if era != "grandpa" and x >= _staircase_x_start \
				and x < _staircase_x_start + y:
