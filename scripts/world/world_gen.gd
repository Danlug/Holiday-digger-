...
## огород закрывается, ход в шахту теперь через люк, и дырявая поверхность
## перестаёт что-либо значить, кроме как выглядеть брошенной.
##
## Зафиксированные клетки (лестница, сценарное золото) не трогаем: выкопанное
## золото не должно отрасти обратно — игрок его уже унёс.
