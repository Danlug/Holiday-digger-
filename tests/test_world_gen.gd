...
func test_staircase() -> void:
	...
	var w := WorldGen.new(1)
	# Самый широкий уровень — первый (сразу под поверхностью): герой должен
	# видеть всю запертую полосу целиком, ещё стоя наверху, а не по одной
	# клетке за раз (см. отчёт агента у world_gen.gd:_pre_autodig_min_x —
	# баг владельца "не копается даже справа от лестницы"). Сужается вниз.
	var expected_counts := {1: 4, 2: 3, 3: 2, 4: 1}
	var ok := true
	...
	check("количество клеток лестницы по уровням 1/2/3/4 (широко сверху, узко снизу)", ok)
...
func test_pre_autodig_gate() -> void:
	...
	# Реальная копка: (18, 2) вне самой лестницы (на втором уровне она теперь
	# самая широкая СВЕРХУ и занимает только x=15..17 — см. отчёт агента у
	# world_gen.gd:_pre_autodig_min_x), поэтому без гейта копается свободно, а
	# с гейтом — только из-за него, не из-за типа клетки. (16,1) для этой
	# проверки больше не годится: после разворота лестницы он САМ стал её
	# частью (нерушим всегда, гейт тут ни при чём) — это отдельно проверяется
	...
	# (16,1) теперь часть самой лестницы (широкий верхний уровень) — не
	# копается вообще, гейт тут ни при чём (нерушимая структура, ГДД).
	check("(16,1) — сама лестница, не копается даже гейтом (структура)",
		not w3.dig_cell(16, 1))

	w3.unlock_pre_autodig_garden()
	check("после unlock (18,2) снова копается", w3.dig_cell(18, 2))
	check("после unlock (16,1) по-прежнему не копается — это лестница, не гейт",
		not w3.dig_cell(16, 1))
