...
func test_staircase() -> void:
	...
	var w := WorldGen.new(1)
	# Самый широкий уровень — последний/нижний (владелец, 2026-09-21: «ты
	# зря развернул, я не могу вылезти» — широкий верх сплошняком перекрывал
	# весь проход и герой не мог подняться на поверхность). Узкий верх (y=1,
	# 1 клетка у самого дома) с открытым воздухом над каждой следующей
	# ступенью — так лестница и читается настоящей лестницей, по которой
	# можно подняться, а не перевёрнутым клином без единого прохода.
	...
	check("количество клеток лестницы по уровням 1/2/3/4 (узко сверху, широко снизу)", ok)
...
func test_pre_autodig_gate() -> void:
	...
	# Реальная копка: (18, 2) вне самой лестницы (широкий конец у неё ВНИЗУ —
	# владелец, 2026-09-21: «ты зря развернул, я не могу вылезти», см.
	# world_gen.gd:_pre_autodig_min_x — на втором уровне лестница занимает
	# только x=15..16), поэтому без гейта копается свободно, а с гейтом —
	# только из-за него, не из-за типа клетки. (15,1) — сама лестница на
	# самом узком, верхнем уровне (нерушима всегда, гейт тут ни при чём) —
	# это отдельно проверяется ниже отдельной строкой.
	var w2 := WorldGen.new(777)
	check("без гейта (18,2) копается", w2.dig_cell(18, 2))

	var w3 := WorldGen.new(777)
	w3.lock_pre_autodig_garden()
	check("с гейтом (18,2) не копается", not w3.dig_cell(18, 2))
	check("выкопать так и не удалось — клетка не EMPTY", w3.get_tile(18, 2) != TileTypes.Type.EMPTY)
	check("с гейтом (19,1), сразу правее запертой полосы, копается", w3.dig_cell(19, 1))
	check("под домом отказ по-прежнему свой (x=10, y=1)", not w3.dig_cell(10, 1))
	# (15,1) — узкий верхний конец лестницы, часть структуры на любом
	# уровне — не копается вообще, гейт тут ни при чём.
	check("(15,1) — сама лестница, не копается даже гейтом (структура)",
		not w3.dig_cell(15, 1))

	w3.unlock_pre_autodig_garden()
	check("после unlock (18,2) снова копается", w3.dig_cell(18, 2))
	check("после unlock (15,1) по-прежнему не копается — это лестница, не гейт",
		not w3.dig_cell(15, 1))
