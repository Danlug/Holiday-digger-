...
Чем эта нарезка отличается от import_dig.py:
  ...
  4. Якорь по горизонтали — тело, а не рамка кадра. Художник ставит фигуру
     ...
     верхней полосы содержимого (голова героя, купол кабины) и дополнительно
     подгоняем якоря прямой по методу наименьших квадратов: кадры стоят на
     листе с ровным шагом, всё отклонение от прямой — дрожь рисовальщика,
     а не задуманное движение.
