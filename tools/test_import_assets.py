...
def test_backdrops():
    ...
    for name in ("backdrop_fence_white", "backdrop_fence_old"):
        ...
        # Небо над лесом у забора вырезано (за ним гора и живое небо SkyView),
        # а сам лес/забор/грядки — непрозрачны: середина картинки сплошная.
        top_alpha = a[0, :, 3]
        check(f"{name}.png: небо над лесом прозрачно (альфа верхней строки = 0)",
              top_alpha.max() < 1.0)
        mid_alpha = a[a.shape[0] // 2, :, 3]
        check(f"{name}.png: середина (забор) непрозрачна",
              mid_alpha.min() > 250.0)
